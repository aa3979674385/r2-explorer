<template>
  <q-list style="min-width: 100px">
    <q-item clickable v-close-popup @click="openObject">
      <q-item-section>{{ $t('files.open') }}</q-item-section>
    </q-item>
    ...
    <q-item clickable v-close-popup @click="renameObject" v-if="prop.row.type === 'file'">
      <q-item-section>{{ $t('files.renameFile') }}</q-item-section>
    </q-item>
    <q-item clickable v-close-popup @click="duplicateObject">
      <q-item-section>{{ $t('files.duplicate') }}</q-item-section>
    </q-item>
    <q-item clickable v-close-popup @click="updateMetadataObject" v-if="prop.row.type === 'file'">
      <q-item-section>{{ $t('files.updateMetadata') }}</q-item-section>
    </q-item>
    <q-separator />
    <q-item clickable v-close-popup @click="createShareLink" v-if="prop.row.type === 'file'">
      <q-item-section>
        <q-item-label>{{ $t('files.createShareLink') }}</q-item-label>
        <q-item-label caption>{{ $t('files.shareLinkDesc') }}</q-item-label>
      </q-item-section>
    </q-item>
    <q-item clickable v-close-popup @click="copyInternalLink">
      <q-item-section>
        <q-item-label>{{ $t('files.copyInternalLink') }}</q-item-label>
        <q-item-label caption>{{ $t('files.internalLinkDesc') }}</q-item-label>
      </q-item-section>
    </q-item>
    <q-item clickable v-close-popup @click="copyPublicUrl" v-if="prop.row.type === 'file' && bucketPublicUrl">
      <q-item-section>
        <q-item-label>{{ $t('files.copyPublicUrl') }}</q-item-label>
        <q-item-label caption>{{ $t('files.publicLinkDesc') }}</q-item-label>
      </q-item-section>
    </q-item>
    <q-separator />
    <q-item clickable v-close-popup @click="deleteObject">
      <q-item-section>{{ $t('files.deleteFile') }}</q-item-section>
    </q-item>
  </q-list>
</template>
<script>
