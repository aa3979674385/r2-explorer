<template>
  <q-dialog v-model="deleteModal" @hide="reset">
    <q-card>
      <q-card-section class="row column" v-if="row">
        ...
        <span v-if="row.type === 'folder'" class="q-ml-sm">{{ $t('files.confirmDeleteFolder') }} <code>{{row.name}}</code>，{{ $t('files.andFilesInside') }}
          <code v-if="deleteFolderInnerFilesCount !== null">{{deleteFolderInnerFilesCount}}</code>
          <code v-else><q-spinner color="primary"/></code>
          {{ $t('files.fileInside') }}</span>
        <span v-else class="q-ml-sm">{{ $t('files.confirmDeleteFile') }} <code>{{row.name}}</code>？</span>
      </q-card-section>
  ...
  <q-dialog v-model="updateMetadataModal" @hide="reset">
    <q-card style="min-width: 300px;">
      <q-card-section class="row column" v-if="row">
        <h6 class="q-mt-none q-mb-sm flex">{{ $t('files.httpMetadata') }} <q-btn class="q-mr-none q-ml-auto" round size="sm" color="primary" icon="add" @click="updateHttpMetadata.push({key: '', value: ''})" /></h6>
        <div class="flex row" v-for="(val, index) in updateHttpMetadata" :key="index">
          <div>
            <q-input v-model="updateHttpMetadata[index].key" :label="$t('files.key')" />
          </div>
          <div>
            <q-input v-model="updateHttpMetadata[index].value" :label="$t('files.value')" />
          </div>
          <div class="flex">
        ...
        <h6 class="q-mt-xl q-mb-sm flex">{{ $t('files.customMetadata') }} <q-btn class="q-mr-none q-ml-auto" round size="sm" color="primary" icon="add" @click="updateCustomMetadata.push({key: '', value: ''})" /></h6>
        <div class="flex row" v-for="(val, index) in updateCustomMetadata" :key="index">
          <div>
            <q-input v-model="updateCustomMetadata[index].key" :label="$t('files.key')" />
          </div>
          <div>
            <q-input v-model="updateCustomMetadata[index].value" :label="$t('files.value')" />
          </div>
          <div class="flex">
      ...
      <q-card-actions align="right">
        <q-btn flat :label="$t('cancel')" color="primary" v-close-popup />
        <q-btn flat :label="$t('files.updateMetadata')" color="orange" :loading="loading" @click="updateConfirm" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
	...
	methods: {
		...
		duplicateObject: async function (row) {
			if (row.type === "folder") {
				...
				const notif = this.q.notify({
					group: false,
					spinner: true,
					message: this.$t('files.duplicatingFolder'),
					caption: "0%",
					timeout: 0,
				});
				...
				notif({
					...
					spinner: false,
					caption: "100%",
					message: this.$t('files.folderDuplicated'),
					timeout: 2500,
				});
			} else {
				...
				this.q.notify({
					...
					icon: "done",
					spinner: false,
					message: this.$t('files.fileDuplicated'),
					timeout: 2500,
				});
			}
		...
		renameConfirm: async function () {
			...
			this.q.notify({
				...
				message: this.$t('files.fileRenamed'),
		...
		updateConfirm: async function () {
			...
			this.q.notify({
				...
				icon: "done", // we add an icon
				spinner: false, // we reset the spinner setting so the icon can be displayed
				message: this.$t('files.fileUpdated'),
				timeout: 2500, // we will timeout it in 2.5s
			});
		},
		deleteConfirm: async function () {
			if (this.row.type === "folder") {
				...
				const notif = this.q.notify({
					group: false,
					spinner: true,
					message: this.$t('files.deletingFiles'),
					caption: "0%",
					timeout: 0,
				});
				...
				notif({
					...
					spinner: false, // we reset the spinner setting so the icon can be displayed
					caption: "100%",
					message: this.$t('files.folderDeleted'),
					timeout: 2500, // we will timeout it in 2.5s
				});
			} else {
				this.deleteModal = false;
				...
				this.q.notify({
					...
					icon: "done", // we add an icon
					spinner: false, // we reset the spinner setting so the icon can be displayed
					message: this.$t('files.fileDeleted'),
					timeout: 2500, // we will timeout it in 2.5s
				});
			}
