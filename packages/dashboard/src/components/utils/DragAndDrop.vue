	...
	methods: {
		...
		async uploadFiles(folders) {
			...
			const notif = this.q.notify({
				group: false,
				spinner: true,
				message: `${this.$t('dragDrop.uploadingFiles')} 1/${filenames.length}...`,
				caption: "0%",
				timeout: 0,
			});
			...
			for (const [folder, files] of Object.entries(folders)) {
				...
				for (const file of files) {
					...
					notif({
						message: `${this.$t('dragDrop.uploadingFiles')} ${uploadCount}/${filenames.length}...`,
					});
			...
			notif({
				...
				caption: "100%",
				message: this.$t('dragDrop.filesUploaded'),
