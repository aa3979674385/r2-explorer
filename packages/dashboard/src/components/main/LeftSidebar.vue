<template>
  <div class="q-pa-md" style="height: 100%">
    <div class="flex column" style="height: 100%">
      <q-btn v-if="mainStore.apiReadonly" color="red" stack class="q-mb-lg" :label="$t('layout.readOnly')" />
      <q-btn v-else color="green" icon="add" stack class="q-mb-lg" :label="$t('layout.new')">
        <q-menu>
          <q-list>
      ...
      <q-btn class="q-mb-sm" @click="gotoFiles" color="blue" icon="folder_copy" :label="$t('layout.files')" stack />
      <q-btn v-if="mainStore.config && mainStore.config.emailRouting !== false" class="q-mb-sm" @click="gotoEmail" color="blue" icon="email" :label="$t('layout.email')" stack />
  ...
  <q-dialog v-model="infoPopup" persistent no-route-dismiss>
    <q-card>
      <q-card-section>
        <div class="text-h6">🎉 {{ $t('layout.thankYou') }} 🚀</div>
      </q-card-section>

      <q-card-section class="q-pt-none">
        {{ $t('layout.version') }} <b>{{ mainStore.version }}</b><br>
        ...
        <template v-if="mainStore.auth">
          <b>{{ $t('layout.authentication') }}</b><br>
          Method: {{ mainStore.auth.type }}<br>
          Username: {{ mainStore.auth.username }}
        </template>
        <template v-else>
          {{ $t('layout.notAuthenticated') }}
        </template>
        <br><br>
        <b>{{ $t('layout.serverConfig') }}</b><br>
