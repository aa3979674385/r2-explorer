...
<!--            </q-btn>-->
      ...
      <q-card-section horizontal class="q-px-sm">
        ...
        <div class="q-ml-auto">
          <small class="">{{ file.date }} ({{timeSinceLocal(new Date(file.date))}})</small>
        </div>
      </q-card-section>
	...
	methods: {
		...
		markAsUnread: async function () {
			...
			this.q.notify({
				...
				message: this.$t('email.emailMarkedUnread'),
		...
		markAsRead: async function () {
			...
			this.q.notify({
				...
				icon: "done", // we add an icon
				spinner: false, // we reset the spinner setting so the icon can be displayed
				message: this.$t('email.emailMarkedRead'),
				timeout: 2500, // we will timeout it in 2.5s
			});
		},
